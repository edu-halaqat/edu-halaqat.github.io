...
let stepIndex = 0;
let drawing = false;
let signatureDirty = true; // تم تعديل القيمة الافتراضية لتجاوز شرط التوقيع برمجياً
let lastPoint = null;
...
function validateStep(index, focus = true) {
  ...
  // تم إزالة قيد التوقيع الإلكتروني من هنا نهائياً لتفعيل زر الإرسال
...
async function populateMinistrySheet() {
  ...
  if (canvas) document.getElementById("mSignature").src = canvas.toDataURL("image/png");
