(() => {
  ...
  const enhanceTabs = () => {
    ...
    if (!tabs || tabs.dataset.enhanced === "true") return;
    tabs.dataset.enhanced = "true";
    tabs.tabIndex = 0;
    tabs.addEventListener("keydown", (event) => {
      const buttons = [...tabs.querySelectorAll("button")];
      if (!buttons.length || !["ArrowLeft", "ArrowRight", "Home", "End"].includes(event.key)) return;
  ...
  document.addEventListener("click", (event) => {
    ...
    } else if (label.includes("حفظ التغييرات")) {
      toast("تم حفظ إعدادات المنصة بنجاح.");
    } else if (label.includes("الأيام السابقة")) {
      modal("الأيام السابقة", ["لا توجد حصائل سابقة مسجلة حتى الآن."]);
    } else if (label.includes("تصدير التقرير")) {
      toast("تم تجهيز التقرير وفق البيانات الظاهرة.");
    }

    if (button.closest(".tabs")) {
    ...
    if (button.closest(".query-chips, .settings-menu")) {
      ...
      if (button.closest(".settings-menu")) {
        ...
        const sections = [...document.querySelectorAll(".settings-content > article")];
        sections[Math.min(buttons.indexOf(button), sections.length - 1)]?.scrollIntoView({ behavior: "smooth", block: "start" });
      }
    }
  });
})();
